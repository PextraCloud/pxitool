...
func New(volumeType volumetype.VolumeType, volumeId string) *SVOL {
	...
	dataLen := 1 + 1 + volumeIdLen + 4 // 1 byte for volume type, 1 byte for volume ID length, len(volumeId) bytes for volume ID, 4 reserved bytes
	c := &SVOL{
		Chunk: chunk.Chunk{
	...
	c.Data[1] = uint8(volumeIdLen)

	volumeIdBytes := []byte(volumeId)
	copy(c.Data[2:2+len(volumeIdBytes)], volumeIdBytes)
	// Zero out reserved bytes
	copy(c.Data[2+len(volumeIdBytes):2+len(volumeIdBytes)+4], []byte{})
...
func GetDataStruct(data []byte) (*Data, error) {
	...
	volumeId := string(data[2 : 2+volumeIdLen])
	reserved := [4]byte{}
	copy(reserved[:], data[2+volumeIdLen:])

	if err := verifyReservedBytes(reserved); err != nil {
		return nil, err
	}

	// 1 byte volume type, 1 byte volume ID length, volumeIdLen bytes for volume ID, 4 reserved bytes, 4 bytes CRC32 (zeroed in this case)
	offset := 1 + 1 + volumeIdLen + 4 + 4
	return &Data{
